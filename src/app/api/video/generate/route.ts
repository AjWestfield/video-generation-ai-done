...
export async function POST(request: Request) {
  try {
    const { images, timedImages, audioBase64, duration = 5 } = await request.json();
    ...
    // FFmpeg command to create video from images and audio with fixed 1920x1080 aspect ratio
    // Use precise settings to ensure exact 1920x1080 output without any aspect ratio changes
    const ffmpegCommand = `${process.env.FFMPEG_PATH || 'ffmpeg'} -y -f concat -safe 0 -i ${inputListPath} -i ${audioPath} -c:v libx264 -vf "scale=1920:1080:force_original_aspect_ratio=1,setsar=1:1,zoompan=z='min(zoom+0.0015,1.05)':d=100:s=1920x1080" -pix_fmt yuv420p -preset fast -r 24 -c:a aac -b:a 192k -shortest ${outputVideoPath}`;
