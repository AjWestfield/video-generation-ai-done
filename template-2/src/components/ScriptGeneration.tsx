...
const ScriptGeneration: React.FC<ScriptGenerationProps> = ({
  ...
  const generateScript = async () => {
    ...
    try {
      // Determine which API endpoint to call based on narrative mode
      const endpoint = narrativeMode 
        ? "/api/openrouter/generate-narrative-script" 
        : "/api/openrouter/generate-script";
      
      const payload = narrativeMode 
        ? { 
            prompt: videoIdea, 
      ...
      <div className="space-y-3">
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[rgba(var(--accent-cyan),1)] to-[rgba(var(--accent-blue),1)] text-glow">
            Generating {narrativeMode ? 'Narrative ' : ''}Script
          </h2>
          <p className="text-gray-300 mt-1 text-sm">
            Our AI is creating an engaging {videoDuration}-minute{narrativeMode ? ' storytelling' : ''} script based on your idea...
          </p>
          {narrativeMode && (
            <p className="text-[rgba(var(--accent-purple),0.9)] mt-1 text-xs">
              Using {getStoryStructureName(storyStructure)} structure
            </p>
          )}
          {retryCount > 0 && (
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <h2 className="text-xl md:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[rgba(var(--accent-cyan),1)] to-[rgba(var(--accent-blue),1)] text-glow">
          Your {narrativeMode ? 'Storytelling ' : ''}Script
        </h2>
        <p className="text-gray-300 mt-1 text-xs md:text-sm">
          Review the script generated by AI. You can proceed, edit, or regenerate it.
        </p>
        {narrativeMode && (
          <p className="text-[rgba(var(--accent-purple),0.7)] text-xs mt-1">
            Using {getStoryStructureName(storyStructure)} narrative structure
          </p>
        )}
        {wordCount && (
          <div>
...
// Helper function to get a friendly name for the story structure
function getStoryStructureName(structure: string): string {
  ...
  return structureNames[structure] || "Standard";
}
